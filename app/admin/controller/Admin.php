...
class Admin extends Common
{
    ...
    protected function _initialize()
    {
        ...
        $this->assign('uid', ADMIN_ID);
        $this->assign('roleid', ADMIN_ROLE);
        ...
        if (!$this->request->isAjax()) {
            ...
            if ( ADMIN_ROLE == 3 and $project = db("project_info")->where('cuser', $login['uid'])->find() ) {
                // if ($project['step'] >= 30) {
                //     $this->assign('headshow', 1);
                // } else {
                    $this->assign('headshow', 0);
                // }
                // $this->assign('headstep', $project['step']);
                // $this->assign('headproid', $project['recid']);
            } else {
                $this->assign('headshow', 1);
            }

            if ( ADMIN_ROLE == 5 ) {
               $this->assign('headhome', 0); 
            } else {
                $this->assign('headhome', 1); 
            }
        }
    }
