...
class User extends Admin
{
    ...
    public function index($q = '')
    {
        ...
        if ($role_id == 1) {  //超级管理员
           $where .= " and id != 1"; 
        } else  if ($role_id == 2) {  //系统管理员
            $where .= " and role_id != 1"; 
        } else {
            $cdata = db("admin_user")->where('cuser', $login['uid'])->field("id")->select();
            if (count($cdata) > 0) {
                $findUser = array();
                foreach($cdata as $k=>$v) {
                    $findUser[] = $v['id'];
                }
                $where .= " and id in (". implode(",", $findUser) .")";    
            } else {
                $where .= " and id in (0)";  
            }  
        }
        $data_list = UserModel::where($where)->order('role_id asc')->paginate();
        // 分页
    ...
    public function addUser()
    {
        ...
        $this->assign('xiaquData', json_decode(cache('dict_dict_xiaqu'), true));
     
        $this->assign('departmentData', json_decode(cache('dict_dict_department'), true));

        $this->assign('upostData', json_decode(cache('dict_dict_upost'), true));

        $stypeArr = array(
                            1=>array('recid'=>1,'title'=>'政府职能部门'),
                            2=>array('recid'=>2,'title'=>'手续办理部门'),
                            3=>array('recid'=>3,'title'=>'办事处'),
                            4=>array('recid'=>4,'title'=>'企业'),
                            99=>array('recid'=>99,'title'=>'其他'),
                        );

        $this->assign('stypeArr', $stypeArr);
        $this->assign('tab_data', $tab_data);
    ...
    public function editUser($id = 0)
    {
        ...
        $row = UserModel::where('id', $id)->field('id,username,role_id,nick,email,mobile,auth,status,unit,department,upost,xiaqu,stype')->find()->toArray();
        if (!$row['auth']) {
