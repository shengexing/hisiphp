...
<div class="page-form">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            {if condition="$examData.status eq 1"}<input readonly type="text" class="layui-input layui-noborder" value='未提交'>{/if}

            {if condition="$examData.status eq 2"}<input readonly type="text" class="layui-input layui-noborder" value='进行中'>{/if}

            {if condition="$examData.status eq 3"}<input readonly type="text" class="layui-input layui-noborder" value='已完成'>{/if}

             <input type="hidden" name="status" value="{$examData.status}">
        </div>
    </div>
        ...
        <div class="layui-input-inline" style="width:750px">
            {volist name="unitData" key = "pk" id="pt"}
                <input type="checkbox" value="{$pt.recid}" 
                    class="layui-input field-capital capital-checkbox" {if condition="$examUnit[$pt.recid]['check'] eq 1"}checked{/if} name="qespunit[]">{$pt.title}
            {/volist}
        </div>
    </div>
...
<script>
var formData = {:json_encode($examData)};
