...
<div class="page-form">
            ...
            <div class="layui-input-inline">
                <select name="pid" type="select" lay-verify="required">
                    ...
                    {volist name="pdata" key = "pk" id="pt"}
                        ...
                        <option value="{$pk}" {if condition="$data['pid'] eq $pk"} selected {/if}>{$pt['title']}</option>
                        {/if}
                    {/volist}
                </select>
            </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" class="layui-input field-username" value="{$data['title']}" name="title" lay-verify="required" autocomplete="off" placeholder="名称（必填）">
        </div>
    </div>

   {if condition="$type eq 3 or $type eq 4 or $type eq 6"}
    ...
    {/if}
     ...
     {if condition="$type eq 4"}
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="radio" class="field-type" name="type" value="1" title="重大产业项目" {if condition="$data['type'] eq 1"}checked{/if}>
                <input type="radio" class="field-type" name="type" value="2" title="十类重点工程" {if condition="$data['type'] eq 2"}checked{/if}>
            </div>
        </div>
    ...
    {if condition="$type eq 5 or $type eq 6"}
         <div class="layui-form-item">
        <label class="layui-form-label layui-form-project">排序：</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input field-sorder" value="{$data['sorder']}" name="sorder" lay-verify="required" autocomplete="off" placeholder="排序">
        </div>
    </div>
    {/if}
     ...
     <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="radio" class="field-status" name="status" value="1" title="启用" {if condition="$data['status'] eq 1"}checked{/if}>
            <input type="radio" class="field-status" name="status" value="0" title="禁用" {if condition="$data['status'] eq 0"}checked{/if}>
        </div>
    </div>
